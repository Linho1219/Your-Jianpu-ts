...
function applyTransform(box: BBox, transform: Transform): BBox {
  ...
  return new BBox(box.x1 * sx + dx, box.y1 * sy + dy, box.x2 * sx + dx, box.y2 * sy + dy);
}
...
export function getBoundingBox(
  ...
  if (tree.type === 'Leaf') {
    ...
    return new BBox(-ax * width, -ay * height, (1 - ax) * width, (1 - ay) * height);
...
export const getBoundingBoxWithCfg = (config: RenderConfig) => (tree: LayoutTree<RenderObject>) =>
